...
CREATE Procedure [dbo].[FullFeatures_14010319]
AS 
set nocount on
...
(
	...
	,max(CandleCeiling) over(order by RowNo rows between 23 preceding and 9 following) as MaxBetween  -- Bazeye 23 + 23 baraye shenasaee Price pivot MAX  
	,Min(CandleFloor) over(order by RowNo rows between 23 preceding and 9 following) as MinBetween 	-- Bazeye 23 + 23 baraye shenasaee Price pivot MIN  
	from #TBCUR	
) 
, stp2 as 
(
...
SELECT *
	...
	,(a.[High]-a.[Low])/a.[Close] as PriceRangeWidthPrcnt
...
(
SELECT *
	,(CandleCeiling - CandleFloor) / [Close] as CandleBodyRangeWidthPrcnt --- اندازه بدنه کندل 
	,(ABS([close]-[open])/[_PriceRange])		as [_CloseOpenPrcnt]
	,([High]-CandleCeiling)/[_PriceRange]		as [_HighCielingPrcnt]
	,(CandleFloor-[Low])/[_PriceRange]			as [_FloorLowPrcnt]
FROM stp1 s
),
Stp3 as 
(
...
Declare @ActiveAlgorithmPips int = 10
declare @PivotFollowing int = 10
...
SELECT 
	RowNo
	,[DateTime]
	,DATEADD(HOUR,@PivotFollowing,[DateTime]) AS FromDate
...
(
	...
	,f.RowNo,F.DateTime,F.[TIME],F.[Close],F.[High],F.CCI,F.Endt,ROW_NUMBER()OVER(PARTITION BY F.ENDT ORDER BY F.[TIME]) RN
	FROM ##ResFin F 
	INNER JOIN #MX M ON F.[DateTime] BETWEEN M.FromDate AND M.Todate
	WHERE ABS(F.[High]*10000-M.[High]*10000)<@ActiveAlgorithmPips--pip
	--and RSI14 > 70
)
...
select m.*,f.[Close] as FClose,m.[Close]*10000-f.[Close]*10000 as Diff,f.[DateTime] as FDateTime into #MySignals2
...
select m.*,f.[Close] as FClose,m.[Close]*10000-f.[Close]*10000 as Diff,f.[DateTime] as FDateTime into #CheckResult
...
select *,@StopLosPips as Pip,'StopLoss' as OprType,'MaxPvt' as TradeType into ##CheckResult
	--count(1)*@StopLosPips -- -2380
from #MySignals2 t where exists (select 1 from #CheckResult c where Diff <= @StopLosPips and c.RowNo = t.RowNo)
...
;WITH StopLoss as 
(select RowNo,min(c.FDateTime) as StopLossMinDateTime from #CheckResult c where Diff <= @StopLosPips 
	group by RowNo)
, Tgt as
...
-- 30 معامله
insert into ##CheckResult
select *,@StopLosPips,'StopLoss' as OprType,'MaxPvt' as TradeType
...
select * ,diff,'Between' as OprType,'MaxPvt' as TradeType
	...
	from #MySignals2 t where not exists (select 1 from #CheckResult c where Diff <= @StopLosPips and c.RowNo = t.RowNo)
	and not exists (select 1 from #CheckResult c where Diff >=@TargetsPips and c.RowNo = t.RowNo)
...
SELECT 
	RowNo
	,[DateTime]
	,DATEADD(HOUR,10,[DateTime]) AS FromDate
	,LEAD([DateTime],2,GETDATE()+100) OVER(ORDER BY [DateTime]) AS Todate
	,f.[Low]
INTO #MN
...
(
	...
	,f.RowNo,F.DateTime,F.[TIME],F.[Close],F.[Low],F.CCI,F.Endt,ROW_NUMBER()OVER(PARTITION BY F.ENDT ORDER BY F.[TIME]) RN
	FROM ##ResFin F 
	INNER JOIN #MN M ON F.[DateTime] BETWEEN M.FromDate AND M.Todate
...
select m.*,f.[Close] as FClose,f.[Close]*10000-m.[Close]*10000 as Diff,f.[DateTime] as FDateTime into #MyMinSignals
from #MyMinSignals0 m inner join ##ResFin f on f.RowNo = m.RowNo+@Candles


-- حالا چک میکنیم ببینیم با حد ضرر 2 پیپ و حد سود 10 پیپ چی میشده تو این بازه 20 کندل
DROP TABLE IF EXISTS #CheckMnResult
select m.*,f.[Close] as FClose,f.[Close]*10000-m.[Close]*10000 as Diff,f.[DateTime] as FDateTime into #CheckMnResult
...
-- 78 معامله
insert into ##CheckResult
select *,@StopLosPips,'StopLoss' as OprType,'MinPvt' as TradeType
	--count(1)*@StopLosPips -- -2770
from #MyMinSignals t where exists (select 1 from #CheckMnResult c where Diff <= @StopLosPips and c.RowNo = t.RowNo)
...
;WITH StopLoss as 
(select RowNo,min(c.FDateTime) as StopLossMinDateTime from #CheckMnResult c where Diff <= @StopLosPips 
	group by RowNo)
, Tgt as
...
select * ,Diff,'Between' as OprType,'MinPvt' as TradeType
	...
	from #MyMinSignals t where not exists (select 1 from #CheckMnResult c where Diff <= @StopLosPips and c.RowNo = t.RowNo)
	and not exists (select 1 from #CheckMnResult c where Diff >=@TargetsPips and c.RowNo = t.RowNo)



drop table if exists CheckResult
select * into CheckResult from ##CheckResult
